...
from .log_server import HedgeLog
from .utils import make_logback_config
...
class replica_engine(object):
			...
			if replica_status in ['syncing', 'running', 'initialising']:
				...
				if self.args.debug:
					...
					if self.config["log_dest"] == 'stdout':
						...
						replica_daemon = Daemonize(app=app_name, pid=replica_pid, action=self.run_replica,
	...
	def __init_logger(self):
		...
		log_port = self.config['hedgelog']["port"]
		log_host = self.config['hedgelog']["host"]
		...
		if log_dest == 'stdout' or debug_mode:
			...
			fh = SocketHandler(log_host, log_port)
	...
	def start_logger(self):
		# Assuming that there's only one logger is active

		log_params = self.config['hedgelog']
		log_dir = self.config["log_dir"]
		log_level = self.config["log_level"]
		log_days_keep = self.config["log_days_keep"]
		log_config_path = os.path.expanduser(log_params['config'])

		config_name = self.args.config
		source_name = self.args.source
		debug_mode = self.args.debug
		if source_name == '*':
			log_name = "%s_general.log" % config_name
		else:
			log_name = "%s_%s.log" % (config_name, source_name)
		...
		if not os.path.exists(log_config_path):
			_logger_d = {	"level": log_level.upper(),
							"propagate": False,
							"handlers": ["charm_log_handler"]}

			_handler_d = {	"class": "logging.handlers.TimedRotatingFileHandler",
							"filename": os.path.join(os.path.expanduser(log_dir), log_name),
							"level": log_level.upper(),
							"formatter": "cham_formatter",
							"when": "d",
							"interval": 1,
							"backupCount": log_days_keep}

			_formatter_d = {
				"format": str_format,
				"datefmt": "%Y-%m-%d %H:%M:%S"
			}
			make_logback_config(_formatter_d, _logger_d, _handler_d, log_config_path, __name__)

		h = HedgeLog(log_params['host'], log_params['port'], log_config_path)
		h.start_server()

	def stop_logger(self):
		log_params = self.config['hedgelog']
		h = HedgeLog(log_params['host'], log_params['port'], log_params['config'])
		h.stop_server()
